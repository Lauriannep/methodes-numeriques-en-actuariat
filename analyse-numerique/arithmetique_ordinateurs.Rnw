...
\gotorbox{Exécuter le code informatique de la
  ...
  correctement. Les exemples sont tous tirés de véritables messages
  envoyés à la liste de discussion
  \link{https://stat.ethz.ch/mailman/listinfo/r-help/}{\texttt{r-help}}.}
...
\texttt{r-help} sont invariablement renvoyés à
l'\link{http://cran.r-project.org/doc/FAQ/R-FAQ.html\#Why-doesn_0027t-R-think-these-numbers-are-equal_003f}{entrée
  7.31} de la foire aux questions de R.
...
\tipbox{Tel que mentionné ci-dessus, il est tout à fait
  possible de se passer de la conversion de la partie fractionnaire.
  Pour convertir le nombre réel $x$ en base $b$ avec un maximum de $n$
  chiffres dans la partie fractionnaire, il suffit de:
  \begin{enumerate}
...
\gotorbox{La fonction \code{arrayInd} de R permet de faire exactement ce qui
...
\notebox{Le 1{\ier} décembre 2014, YouTube a annoncé que la
  vidéo de la chanson \emph{Gangnam Style} avait «brisé» son compteur
  en dépassant le nombre maximal de visionnements admissible, soit
...
\tipbox{Toutes les fonctions de calcul de fonction de
  répartition de R permettent d'évaluer plus précisément les
  probabilités $1 - F(x)$ pour une grande valeur de $x$ en spécifiant
  l'option \code{lower.tail = FALSE}.}
...
\def\scriptfilename{arithmetique\string_ordinateurs.R}
...
\begin{exercice}
  \label{ex:ordinateurs:complement}
  La plupart des langages de programmation --- dont R, voir la note au
  bas de la \autopageref{pg:arithmetique:entiers} --- permettent de
  ...
  \begin{sol}
    ...
    \item L'ensemble des nombres admissibles pour le type
      ...
      \end{equation*}
      (C'est le type de nombre qui était utilisé par YouTube pour
      comptabiliser le nombre de visionnements d'une vidéo jusqu'à ce
      que \emph{Gangnam Style} excède $\nombre{2147483647}$. Le nombre
      de visionnements est désormais stocké sous forme d'entier sur
      $64$~bits, ce qui a fait passer le maximum à $2^{63} - 1 =
      \nombre{9223372036854775808}$.)
    \end{enumerate}
  \end{sol}
  \begin{rep}
    \begin{enumerate}
