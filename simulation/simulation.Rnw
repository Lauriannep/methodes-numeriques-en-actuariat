...
\tipbox{Dans Excel (et VBA), il faut nécessairement utiliser la
  méthode de l'inverse. Plusieurs fonctions de quantiles sont
  disponibles; voir la \autoref{sec:simulation:excel_et_al}.}
...
segments pleins). On constate aisément que certaines valeurs $y$
