...
\begin{rem}
  Dans Excel (et VBA), il faut nécessairement utiliser la méthode de
...
\end{align*}
Nous procédons ainsi parce que plusieurs générateurs produisent des
...
\begin{exemple}
  ...
  et en partie continue) dont la fonction de répartition est
  \begin{displaymath}
    F(x) =
    \begin{cases}
      0,5x, & 0 \leq x < 1 \\
...
\begin{rem}
  Puisque, par définition, l'aire sous les densités $f$ et $g$ vaut
  $1$ dans les deux cas, la relation $c g_Y(x) \geq f_X(x)$ ne peut
  être vraie pour tout $x$ que si $c > 1$.
\end{rem}
...
\begin{exemple}
  ...
  \ref{fig:simulation:acceptation-rejet2}. Nous pouvons facilement l'inscrire
...
\section{Modèles actuariels courants}
\label{sec:simulation:modeles-actuariels}

Cette section traite de techniques de simulation pour des
distributions couramment utilisés dans la modélisation des risques en
...
Pour concevoir un algorithme de simulation d'un mélange discret à deux
distributions, il faut bien comprendre que chaque observation provient
de l'une ou de l'autre des distributions. Reste, à chaque fois, à
déterminer de quelle distribution simuler.
...
\begin{algorithme}[Mélange discret de deux distributions]
  ...
  nombre aléatoire de la distribution $F$.
  \begin{enumerate}
  \item Obtenir un nombre $u$ d'une loi $U(0, 1)$.
  \item Si $u \leq p$, générer un nombre aléatoire de la distribution
    $F_1$, sinon générer un nombre aléatoire de $F_2$.
  \end{enumerate}
\end{algorithme}
...
$1$ nombre de la distribution $F_1$ si $u \leq p$ $0$ nombre de la
distribution $F_2$ ou, au contraire, $0$ nombre de $F_1$ si $u > p$ et
$1$ nombre de $F_2$. Cette observation joue un rôle important pour
concevoir une procédure de simulation de $n > 1$ valeurs d'un mélange.

La fonction \code{rmixture} du paquetage \pkg{actuar} \citep{actuar}
offre une interface conviviale pour obtenir des nombres aléatoires de
mélanges discrets.
...
Nous pouvons étendre le concept de mélange discret à un quantité non
...
\begin{definition}
  ...
  \end{equation*}
  Cette distribution distribution est un mélange continu des
...
\begin{algorithme}[Mélange continu de deux distributions]
  ...
  étapes ci-dessous permettent de générer un nombre aléatoire de la
  densité $f_X$.
  \begin{enumerate}
  \item Obtenir un nombre $\theta$ issu de la distribution avec
    densité $u(\theta)$.
  \item Obtenir un nombre $x$ issu de la distribution avec
    densité $f(x|\theta)$.
  \end{enumerate}
\end{algorithme}
...
Le fait que les fonctions de simulation de variables aléatoire de R
sont vectorielles rend la simulation de mélanges continus très simples
à réaliser.

\begin{exemple}
  Soit $X$ une variable aléatoire issue du mélange
  $X|\Theta = \theta \sim \text{Poisson}(\theta)$ et
  $\Theta ~ \text{Gamma}(5, 4)$. Les deux expressions R ci-dessous
...
les modèles hiérarchique devrait aller de soi.
