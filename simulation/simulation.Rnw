...
chaque fois, à déterminer de quelle distribution simuler.
...
\begin{exercice}
  ...
  Faire la mise en œuvre informatique (dans le langage de votre
...
\begin{exercice}
  ...
  \begin{sol}
    ...
    \item Si $X \sim \text{Gamma}(\alpha, 1)$, alors $Y = X/\lambda
      ...
      x/\lambda$.
    \item Si $\alpha$ est entier, nous pouvons générer $\alpha$ nombres
...
\begin{exercice}
  ...
  \begin{sol}
    \begin{enumerate}
    \item Nous avons un mélange continu avec
      $X|\Theta \sim \text{Exponentielle}(\Theta)$ et
      $\Theta \sim \text{Gamma}(\alpha, \lambda)$. Par la loi des
...
\begin{exercice}
  ...
  d'acceptation-rejet ci-dessous pour simuler des observations d'une loi
