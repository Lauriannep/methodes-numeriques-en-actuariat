...
\begin{prob-enonce}
  ...
  On souhaite vérifier que lorsque les quatre points sont distribués
...
l'\link{http://support.microsoft.com/kb/231847}{article 231847} de la
base de connaissances Microsoft, le générateur de nombres aléatoires
utilisé par la fonction \code{RND()} est un simple générateur
congruentiel linéaire.
...
@
  ...
  Pour répéter cette expression un grand nombre de fois, on aura
  recours, tel qu'expliqué à l'\autoref{planification}, à la fonction
  \code{replicate}. Puisque chaque expression exécutée par
  \code{replicate} retourne \code{TRUE} ou \code{FALSE}, le résultat de
<<echo=TRUE, eval=FALSE>>=
replicate(1E5, is.triangle(x = runif(4), y = runif(4)))
@
  sera un vecteur booléen. Comme on le sait, ces valeurs booléennes
  sont converties en valeurs numériques $0$ et $1$ pour les calculs
  algébriques. La valeur moyenne du vecteur booléen correspond donc à
  la proportion d'enveloppes convexes triangulaires au fil des
...
@
  soit une valeur suffisamment proche de $\frac{11}{36} \approx
  0,30556$ pour juger l'expérience concluante.
\end{prob-solution}
...
\begin{exercice}
  ...
  \begin{sol}
    \begin{enumerate}
    \item On utilise de nouveau la fonction \code{rand} de la
...
\begin{exercice}
  ...
  donné les deux autres valeurs. On se contente de constater cet état
  de fait graphiquement.
  \begin{enumerate}
